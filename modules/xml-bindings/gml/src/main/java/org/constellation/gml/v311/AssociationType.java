...
/**
 * A pattern or base for derived types used to specify complex types corresponding to an  unspecified UML association
 * - either composition or aggregation.
 * Restricts the cardinality of Objects contained in the association to a maximum of one.
 * An instance of this type can contain an element representing an Object, or serve as a pointer to a remote Object.
 * 
 ...
 * 
 * In many cases it is desirable to impose the constraint prohibiting the occurence of both reference and value in the same instance, as that would be ambiguous.
 * This is accomplished by adding a directive in the annotation element of the element declaration.
 ...
 * 
 * If co-occurence is not prohibited, then both a link and content may be present.
 * If this occurs in an instance, then the rule for interpretation is that the instance found by traversing the href provides the normative value of the property,
 * and should be used when possible.  The value(s) included as content may be used if the remote instance cannot be resolved.
