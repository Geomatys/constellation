...
/**
 * Store configuration bean.
 *
 * @author Johann Sorel (Geomatys)
 */
public class StoreBean extends I18NBean{
        
    private final String serviceName;
    private final String mainPage;
    private final String sourceConfigPage;        
    private final StoreBean.EditedProvider current = new StoreBean.EditedProvider();
    
    public StoreBean(final String serviceName, final String mainPage, 
            final String configPage, String ... bundles){
    ...
    public StoreBean.EditedProvider getEdited() {
        return current;
    }
    
    public List<StoreBean.ChoiceType> getChoices(){
        final ConstellationServer server = getServer();
        final ProvidersReport report = server.providers.listProviders();
        final ProviderServiceReport service = report.getProviderService(getServiceName());
        final List<ProviderReport> providers = service.getProviders();
        
        final ParameterDescriptorGroup servicedesc = (ParameterDescriptorGroup) server.providers.getServiceDescriptor(getServiceName());
        ...
        final List<StoreBean.ChoiceType> choices = new ArrayList<StoreBean.ChoiceType>();
        for(final GeneralParameterDescriptor c : choice.descriptors()){
            final StoreBean.ChoiceType group = new StoreBean.ChoiceType();
            ...
            for(final ProviderReport provider : providers){
                ...
                for(GeneralParameterValue candidate : choiceparam.values()){
                    if(group.name.equals(candidate.getDescriptor().getName().getCode())){
                        final DefaultMutableTreeNode node = new StoreBean.ProviderNode(param);
                        root.add(node);
    ...
    public class EditedProvider{
        ...
        public void apply(){
            ...
            if(newInstance){
                ...
                server.providers.createProvider(getServiceName(), sourcevalue);
            }else{
                //update an existing provider
                server.providers.updateProvider(getServiceName(), originalId, sourcevalue);
            }
