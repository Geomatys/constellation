...
public final class Font implements StyleElement<org.opengis.style.Font> {
    ...
    public org.opengis.style.Font toType() {
        final Literal family = FF.literal("Arial");
        final Literal style  = FF.literal(this.italic ? "italic" : "normal");
        final Literal weight = FF.literal(this.bold ? "bold" : "normal");
        final Literal size   = FF.literal(this.size);
        return SF.font(family, style, weight, size);
    }
}
