...
public class SeriesMetadataTableTest extends TableTest {
    ...
    public void testSelectAndList() throws CatalogException, SQLException {
        final SeriesMetadataTable table = new SeriesMetadataTable(database);
        final SeriesMetadata entry = table.getEntry(id);
        //assertEquals(id, entry.getName());
        assertSame("Should be cached.", entry, table.getEntry(id));

        final Set<SeriesMetadata> entries = table.getEntries();
