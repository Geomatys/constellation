 ...
 */
final class OperationEntry extends Entry implements Operation {
    /**
     ...
     * et {@code "Interpolate"}). Ce nom est inconnu du {@linkplain #getProcessor() processeur}.
        ...
        for (int i=0; i<3; i++) {
            ...
            switch (i) {
                case 0: {
                    ...
                    param = getProcessor().getOperation("NodataFilter").getParameters();
                    break;
                }
                case 1: {
                    /*
                    ...
                    param = getProcessor().getOperation(operation).getParameters();
    ...
    public synchronized Coverage doOperation(Coverage coverage) {
        for (final ParameterValueGroup param : parameters) {
            ...
            try {
                p.setValue(coverage);
                coverage = getProcessor().doOperation(param);
            } finally {
                p.setValue(null);
            }
        }
